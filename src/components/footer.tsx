...
const Footer = () => {
  return (
    <footer className="py-8 mt-[100px] flex flex-col w-full items-center space-y-6">
      ...
      <span className="text-md text-secondary-foreground">Built with 💖 by Ekep-Obasi</span>
      <div className="flex flex-row space-x-4">
        {socialLinks.map((link) => (
          <Link target="_blank" key={link.link} href={link.link} aria-label={link.ariaLable}>
            {link.icon}
          </Link>
        ))}
      </div>
